import React from 'react';
import { WishData } from '../../types';
import { Upload, Sparkles } from 'lucide-react';

interface Step10ScratchCardProps {
  wish: WishData;
  onChange: (updated: Partial<WishData>) => void;
  onNext: () => void;
}

export const Step10ScratchCard: React.FC<Step10ScratchCardProps> = ({ wish, onChange, onNext }) => {
  const handleCardImageUpload = (file: File) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      if (e.target?.result) {
        onChange({
          scratchCard: {
            ...wish.scratchCard,
            revealImage: e.target.result as string,
          },
        });
      }
    };
    reader.readAsDataURL(file);
  };

  return (
    <div className="w-full max-w-2xl mx-auto p-4 space-y-6 animate-fadeIn pb-12">
      <div className="space-y-1">
        <h2 className="text-3xl font-display font-semibold text-white tracking-wide">
          One last <span className="font-script text-pink-400 font-normal">surprise</span>
        </h2>
        <p className="text-sm text-pink-200/70">
          A foil scratch card that reveals an image and a short message.
        </p>
      </div>

      <div className="space-y-4">
        {/* Title */}
        <div className="space-y-1.5">
          <label className="block text-xs font-bold uppercase tracking-wider text-pink-300">
            TITLE
          </label>
          <input
            type="text"
            value={wish.scratchCard.title}
            onChange={(e) =>
              onChange({
                scratchCard: { ...wish.scratchCard, title: e.target.value },
              })
            }
            placeholder="Happy Birthday! 🎉"
            className="w-full px-4 py-3 bg-[#190e24] border border-pink-500/30 focus:border-pink-500 rounded-xl text-white text-base focus:outline-none focus:ring-2 focus:ring-pink-500/20"
          />
        </div>

        {/* Message */}
        <div className="space-y-1.5">
          <label className="block text-xs font-bold uppercase tracking-wider text-pink-300">
            MESSAGE
          </label>
          <input
            type="text"
            value={wish.scratchCard.message}
            onChange={(e) =>
              onChange({
                scratchCard: { ...wish.scratchCard, message: e.target.value },
              })
            }
            placeholder="You found the hidden surprise 💖"
            className="w-full px-4 py-3 bg-[#190e24] border border-pink-500/30 focus:border-pink-500 rounded-xl text-white text-base focus:outline-none focus:ring-2 focus:ring-pink-500/20"
          />
        </div>

        {/* Reveal Image URL / Upload */}
        <div className="space-y-1.5">
          <label className="block text-xs font-bold uppercase tracking-wider text-pink-300">
            REVEAL IMAGE URL
          </label>
          <div className="flex gap-2">
            <input
              type="url"
              value={wish.scratchCard.revealImage}
              onChange={(e) =>
                onChange({
                  scratchCard: { ...wish.scratchCard, revealImage: e.target.value },
                })
              }
              placeholder="https://..."
              className="flex-1 px-4 py-2.5 bg-[#12071a] border border-pink-500/30 rounded-xl text-white text-xs focus:outline-none focus:ring-2 focus:ring-pink-500/20"
            />
            <label className="px-4 py-2.5 bg-pink-500/20 hover:bg-pink-500/30 border border-pink-500/40 rounded-xl text-xs font-semibold text-pink-200 flex items-center gap-1.5 cursor-pointer transition-all">
              <Upload className="w-3.5 h-3.5" />
              Upload reveal image
              <input
                type="file"
                accept="image/*"
                className="hidden"
                onChange={(e) => {
                  if (e.target.files?.[0]) {
                    handleCardImageUpload(e.target.files[0]);
                  }
                }}
              />
            </label>
          </div>
        </div>

        {/* Scratch Card Preview */}
        {wish.scratchCard.revealImage && (
          <div className="p-4 rounded-2xl border border-pink-500/30 bg-[#160c22] space-y-2">
            <span className="text-xs font-semibold text-pink-300 flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5" /> Hidden Reveal Preview
            </span>
            <div className="relative w-full h-40 rounded-xl overflow-hidden border border-pink-500/40">
              <img
                src={wish.scratchCard.revealImage}
                alt="Reveal"
                className="w-full h-full object-cover"
                onError={(e) => {
                  (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1513151233558-d860c5398176?auto=format&fit=crop&q=80&w=800';
                }}
              />
              <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center p-4 text-center">
                <h3 className="font-display font-bold text-lg text-white drop-shadow">
                  {wish.scratchCard.title}
                </h3>
                <p className="text-xs text-pink-200 drop-shadow mt-1">
                  {wish.scratchCard.message}
                </p>
              </div>
            </div>
          </div>
        )}
      </div>

      <div className="pt-2 flex justify-between items-center">
        <button
          type="button"
          onClick={onNext}
          className="text-xs font-semibold text-pink-300/70 hover:text-white transition-colors"
        >
          Skip
        </button>
        <button
          type="button"
          onClick={onNext}
          className="px-8 py-3 rounded-full bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-semibold shadow-lg shadow-pink-500/25 active:scale-95 transition-all cursor-pointer"
        >
          Next
        </button>
      </div>
    </div>
  );
};
